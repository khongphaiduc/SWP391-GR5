...
<html>
    <head>
        ...
        <style>
            ...
            .form-group.phone-employer {
                display: none;
                margin-top: 10px;
            }

            input[type="radio"][name="role"][value="Employer"]:checked ~ .phone-employer {
                display: block;
            }
        </style>
        <script>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                // Lấy tất cả radio role
                var roleRadios = document.querySelectorAll('input[name="role"]');
                var phoneEmployerGroup = document.getElementById('phoneEmployerGroup');

                function toggleEmployerPhone() {
                    var checked = Array.from(roleRadios).find(r => r.checked)?.value;
                    if (checked === 'Employer') {
                        phoneEmployerGroup.style.display = 'block';
                    } else {
                        phoneEmployerGroup.style.display = 'none';
                        // Optionally, clear the value when hidden
                        phoneEmployerGroup.querySelector('input').value = '';
                    }
                }

                // Cập nhật ngay khi load nếu radio đã được chọn
                toggleEmployerPhone();

                // Thêm sự kiện cho mỗi radio
                roleRadios.forEach(function (radio) {
                    radio.addEventListener('change', toggleEmployerPhone);
                });
            });
        </script>
    </head>
            ...
            <div class="flip-card">
                ...
                <div class="flip-card-back">
                    ...
                    <form action="${pageContext.request.contextPath}/RegisterAccount" method="post" autocomplete="off">
                        ...
                        <div class="phone-employer" id="phoneEmployerGroup">
                            <label for="employer-phone">Số điện thoại liên hệ của Employer</label>
                            <input type="tel" id="employer-phone" name="employerPhone" style="width: 400px" pattern="[0-9]{10,15}" placeholder="Nhập số điện thoại" >
                        </div>
                        <div class="form-group">
